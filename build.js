...
const fs = require('fs');
const pluginName = 'resume_plugin';
...
distFilenames = fs.readdirSync(`${destination}/dist/resume`);
...
const updateLine = (line, name) => {
    const matchedLinePart = line.match(/(?<=resume\/).*?(?=\')/gs).toString();
...
const updatedFileContentArray = pluginFileContents.split(/\r?\n/).map(line => {
    switch (true) {
        case line.includes('wp_enqueue_style( \'ng_resume_styles'):
            return updateLine(line, 'styles');
        case line.includes('wp_register_script( \'ng_resume_main'):
            return updateLine(line, 'main');
        case line.includes('wp_register_script( \'ng_resume_polyfills'):
            return updateLine(line, 'polyfills');
        case line.includes('wp_register_script( \'ng_resume_runtime'):
